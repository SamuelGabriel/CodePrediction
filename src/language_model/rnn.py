...
def dynamic_attention_rnn(cell, inputs, attn_length, num_tasks, batch_size,
    ...
    sequence_length = tf.to_int64(sequence_length)
...
def _rnn_step(time, sequence_length, zero_output, zero_alpha, zero_attn_ids, zero_lmbdas, state, call_cell, state_size):
    ...
    def _copy_some_through(new_output, new_alpha, new_attn_ids, new_lmbdas, new_state):
        ...
        return ([tf.where(copy_cond, zero_output, new_output),
                 tf.where(copy_cond, zero_alpha, new_alpha), # (batch, tasks, k)
                 tf.where(copy_cond, zero_attn_ids, new_attn_ids),
                 tf.where(copy_cond, zero_lmbdas, new_lmbdas)] +
                [tf.where(copy_cond, old_s, new_s)
