...
def shift_tensor(t):
    ...
    shape = tf.shape(t)
    left_start = tf.concat(axis=0, values=[[1], tf.zeros([tf.shape(shape)[0]-1], tf.int64)])
    left_shape = tf.concat(axis=0, values=[[shape[0]-1], shape[1:]])
    zeros = tf.concat(axis=0, values=[[1], shape[1:]])
    t = tf.slice(t, left_start, left_shape)
    t = tf.concat(axis=0, values=[t, tf.zeros(zeros, tf.float32)])
    return t
...
def replace_matrix_row(matrix, index, row_vector, row_size=None):
    ...
    def shift_and_append():
        ...
        return tf.concat(axis=0, values=[left, tf.expand_dims(row_vector, 0)])

    def replace():
        left = tf.slice(matrix, [0, 0], [index, -1])
        right = tf.slice(matrix, [index + 1, 0], [row_size - index - 1, -1])
        return tf.concat(axis=0, values=[left, tf.expand_dims(row_vector, 0), right])
...
def process(_, current):
    count = tf.cast(current[0], tf.int64)
    current = tf.slice(current, [1], [-1])
    max = tf.shape(current)[0]
    sm = tf.expand_dims(tf.slice(current, [max - count], [-1]), 0)
    sm = tf.nn.softmax(sm)
    return tf.concat(axis=0, values=[tf.zeros([max-count]), tf.squeeze(sm, [0])])


def jagged_softmax(input, counts):
    counts = tf.cast(counts, tf.float32)
    inputs = tf.concat(axis=1, values=[counts, input])
